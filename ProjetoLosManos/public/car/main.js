...
let km = -100
const randomTraffic = () => {
    const numCarsCurrent = 2 //Math.floor(Math.random() * 2) + 1;
    ...
    for(let i = 1; i <= numCarsCurrent; i++) {
        const p = i - 1
        const car = new Car(road.getLaneCenter(poss[p]),km,30,50,"DUMMY",2,getRandomColor())
        traffic.push(car)
    }
    km -= 500
}
